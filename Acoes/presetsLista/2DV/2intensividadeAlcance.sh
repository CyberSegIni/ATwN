...
#2)nmap $IP -sV --version-intensitive-all

#FUNCIONAMENTO DO ARQUIVO
echo -n "(2)-->INTENSIVIDADE_ALCANCE"
echo ""
...
then
    ...
    do
    	...
    	echo 
    	((i++))
    	nmap $IP -sV --version-intensitive-all -oX $nomeArquivoSalvar
    	echo
    done
    echo
    ...
    do
    	...
    	echo 
    	((i++))
    	nmap $IP -sV --version-intensitive-all
    	echo
    done
    echo
    echo "Scanner de rede foi realizado $qntscann vezes "
